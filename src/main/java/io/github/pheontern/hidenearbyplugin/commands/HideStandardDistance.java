...
public class HideStandardDistance implements CommandExecutor {

    private final HideNearbyPlugin plugin = HideNearbyPlugin.getPlugin();

    //Changes the default hide distance. Used when player hasn't specified their own. Stored in config.yml.
    @Override
        ...
        if (sender instanceof Player player) {
            ...
            if (player.isOp()) {
                try {
                    ...
                    if (input > 0 && input <= 8) {
                        this.plugin.standardHideDistance = input;
                        player.sendMessage(Component.text("Changed the default hide distance to " + input + " blocks.").color(TextColor.fromHexString("#78f562")));
                        return true;
                    }
                    else if (input == 0){
                        this.plugin.standardHideDistance = input;
